...
export class ExamPageComponent implements AfterViewInit {
  @ViewChild('timerProgressBar', { static: false })
  timerProgressBar!: ElementRef;
  ...
  loadExam(examType: 'test' | 'practice_paper' | string, examId: string) {
    ...
    this.http
      .get(
        Params.SERVICE_BASE_URL + Params.EXAM_SERVICE_URL_SUFFIXS.GET_EXAM,
        options,
      )
      .subscribe({
        next: (result: any) => {
              ...
              this.http
                .get(
                  Params.SERVICE_BASE_URL +
                    Params.EXAM_SERVICE_URL_SUFFIXS.CHECK_SUBMISSION,
                  {
                    headers: headers,
                    params: {
                ...
                .subscribe({
                  next: (value: any) => {
                    ...
                    if (value.success) {
                      if (value.result) {
                        alert(
                          'You have already attempted this exam, so you cannot attempt it again.',
                        );
                        this.location.back();
                      } else {
                        this.getInstructions(this.examDetails.examType);
                        this.loadExam(
                          this.examDetails.examType,
                          this.examDetails.id,
                        ); // TODO ::::
                      }
                    } else {
  ...
  saveAnswer() {
    this.answers[this.selectedQuestionIndex] =
      this.paperDetails!.questions[this.selectedQuestionIndex].userAnswer;
  ...
  getInstructions(examType: string) {
    ...
    this.http
      .get(
        Params.SERVICE_BASE_URL +
          Params.EXAM_SERVICE_URL_SUFFIXS.GET_EXAM_INSTRUCTIONS,
        options,
      )
      .subscribe({
        next: (result: any) => {
  ...
  submitExam() {
    ...
    this.http
      .post(
        Params.SERVICE_BASE_URL + Params.EXAM_SERVICE_URL_SUFFIXS.SUBMIT_EXAM,
        body,
        options,
      )
      .subscribe({
  ...
  changeQuestionsContainerFontSize(method: 'increase' | 'decrease') {
    if (method == 'increase') {
      let currentFontSize: number = Number(
        this.questionContainerUI['font-size'].replace('px', ''),
      );

      if (currentFontSize >= 40) return;
  ...
  goToQuestion(index: number) {
    if (index < 0 || index >= this.paperDetails!.questions.length) return;
  ...
  startExamTimer2(examTime: number) {
    // Not using
  ...
  startExamTimer(examTime: number) {
    ...
    this.examTimerSubscription = source.subscribe(() => {
      ...
      this.setTimerProgress(
        ((examTime * 1000 - remainingTime) * 100) / (examTime * 1000),
      );
    });
  }
...
@Component({
  ...
  template: `
    <i
      class="fa fa-times"
      style="position: absolute; right: 0; margin: 20px; font-size: 24px; cursor: pointer;"
      (click)="close()"
    ></i>
...
@Component({
  ...
  template: `<section>
    <div
      style="display: flex; flex-flow: column; padding: 20px 30px; gap: 20px; font-family: 'Poppins-SemiBold';"
    >
      <h1 style="margin-bottom: 10px; margin-top: 8px;">Submit Paper</h1>

      <img
        src="assets/iconspichon/icons8_question_mark_100px.png"
      ...
      <div style="display: flex; justify-content: space-between; gap: 20px;">
        <button class="close" style="width: 50%;" (click)="close()">
          Close
        </button>
        <button class="btn" style="width: 50%;" (click)="done()">
          Confirm Submit
        </button>
      </div>
    </div>
