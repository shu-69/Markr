...
export class RegisterComponent implements OnInit {
  ...
  submit() {
    if (this.formData.valid) {
      ...
      if (this.selectedProfileImage() != null) {
        ...
        if (imageSize > Params.MAX_PROFILE_IMAGE_SIZE_IN_KB) {
          this.showErrorDialog(
            'Please check your profile image size',
            'Profile image size is greater than allowed image size i.e ' +
      ...
      if (!this.formData.controls.termsnconditions.value) {
        this.showErrorDialog(
          'Please check Tearms and Conditions',
          'Agree to the terms and conditions before submit.',
        );
      ...
      this.http
        .get(
          Params.SERVICE_BASE_URL +
            Params.ACCOUNT_SERVICE_URL_SUFFIXS.CHECK_EMAIL_EXISTS,
          options,
        )
        .subscribe({
            ...
            if (!result) {
              var body: any = {
                ...
                dob: this.datePipe.transform(
                  this.formData.controls.dob.value,
                  'dd-MM-yyyy',
                ),
              ...
              if (
                this.formData.controls.mothername.value != null &&
                this.formData.controls.mothername.value.length > 0
              ) {
                body.mother_name = this.formData.controls.mothername.value;
              }

              if (
                this.formData.controls.aadhar.value != null &&
                this.formData.controls.aadhar.value.length > 0
              ) {
                body.aadhar = this.formData.controls.aadhar.value;
              }

              if (
                this.formData.controls.address.value != null &&
                this.formData.controls.address.value.length > 0
              ) {
              ...
              this.http
                .post<any>(
                  Params.SERVICE_BASE_URL +
                    Params.ACCOUNT_SERVICE_URL_SUFFIXS.REGISTER,
                  body,
                  { headers },
                )
                .subscribe({
                  next: (data) => {
                    ...
                    if (data.status) {
                      ...
                      this.submitButtonText = 'SUBMIT';
                      this.showErrorDialog(
                        'OOPS!, That was not expected!',
                        'Your account cannot be created at this time, please try after some time.',
                      );
                    }
                  },
                  error: (error) => {
              ...
              this.showErrorDialog(
                ...
                'The email you entered is already exists, please use another email or do login insted.',
              );
            }
          },
          error: (error) => {
            ...
            this.showErrorDialog(
              'OOPS!, That was not expected!',
              'Some Internal server error occured, please try after some time.',
            );
          },
        });
    } else {
        ...
        if (controlErrors != null) {
          Object.keys(controlErrors).forEach((keyError) => {
            ...
            switch (key) {
              ...
              case 'dob':
                errorMsg +=
                  '◉' + ' ' + 'DOB' + ' ' + getKeyErrorMessage(keyError) + '\n';
                break;
  ...
  showSuccessDialog(message: string, crenedentials: any) {
    let dialogRef = this.dialog.open(SuccessDialog, {
      ...
      data: {
        message: message,
        username: crenedentials.username,
        password: crenedentials.password,
      },
    });
...
@Component({
  ...
  template: `
    <h1
      mat-dialog-title
      title
      style="text-align: center; font-family: 'Poppins-Bold'; font-size: x-large; color: white; font-weight: normal;
              padding-left: 20px; padding-right: 20px;"
    >
      {{ data.title }}
    </h1>
    <div
      style="text-align: start; font-family: 'Poppins-Regular'; font-size: medium; color: white; font-weight: normal;
              padding-left: 20px; padding-right: 20px; white-space: pre-line; display: flex; justify-content: center;"
    >
      {{ data.message }}
    </div>
    <div
      style="text-align: center;
                          ...
                          background: white;
                          margin-top: 25px;
                          font-family: Poppins-Regular; color: black; "
      (click)="close()"
    >
      Okay!
    </div>
...
@Component({
  ...
  template: `
    ...
    <div
      style="text-align: center; font-family: 'Poppins-Regular'; font-size: medium; color: black; font-weight: normal;
              padding-left: 20px; padding-right: 20px; white-space: pre-line; display: flex; justify-content: center; margin-top: 20px;"
    >
      {{ data.message }}
    </div>
    <div
      style="text-align: center; margin-top: 10px; font-family: Poppins-Regular; font-size: 14px;"
    >
      Login your account with following credentials
    </div>
    <div style="display: flex; justify-content: center;">
      <div
        style="text-align: center; margin-top: 10px; display: flex; flex-flow: column; width: fit-content; text-align: start; gap: 10px; background: #9b9b9b38;
    ...
    <div
      style="text-align: center;
                          ...
                          background: #0176b4;
                          margin-top: 25px;
                          font-family: Poppins-Regular; color: white; "
      (click)="close()"
    >
      Okay!
    </div>
